...
(defn build-keymap [key-man]
  (let [keymap (:new-keymap key-man)]
    ...
    #
    (k "${MOD} + Enter  F" [:summon (match-exe-name "C:\Program Files\Mozilla Firefox\firefox.exe")]
       "Summon Firefox")
    ...
    # window, and shows a message if no matching window can be found.
    #
    (def dev-chrome-jefferson
        ["C:\Program Files\Google\Chrome\Application\chrome.exe" "https://en.wikipedia.org/wiki/Thomas_Jefferson"])
    (k "${MOD} + Enter  Q" [:exec
                            true
                            ;dev-chrome-jefferson]
        "Summon Chrome Jefferson")
...
# # jwno-config.janet (snippet)

# (def url_website_1 "https://en.wikipedia.org/wiki/Thomas_Jefferson") # <-- replace

# (def root-keymap (:new-keymap (in jwno/context :key-manager)))

# (defn chrome-website-1-window? [win]
#   (def exe  (or (in win :exe) ""))
#   (def cls  (or (in win :class) ""))
#   (def name (or (in win :name) ""))
...
#   # Chrome is usually "...\\chrome.exe" and often has class "Chrome_WidgetWin_1"
#   (and
#     (or (string/find (string/lower exe) "chrome.exe")
#         (= cls "Chrome_WidgetWin_1"))
#     ; If your page title includes something distinctive, put it here:
#     ; (string/find name "Some Unique Title")
#     true))

# (:add-command (in jwno/context :command-manager) :open-website-1-here
#   (fn []
#     (:summon chrome-website-1-window? true
#              (string "cmd /c start \"\" \"" url_website_1 "\""))))
             
# # Multi-level key binding:
# # Press/release Win+Enter, then press/release Ctrl+F2. :contentReference[oaicite:1]{index=1}
# (:define-key root-keymap "Win + Enter  Ctrl + F2" :open-website-1-here)

# # Optional: a “cancel” key for the prefix layer (acts like backing out). :contentReference[oaicite:2]{index=2}
# (:define-key root-keymap "Win + Enter  Escape" :nop)

# # Activate your root keymap. :contentReference[oaicite:3]{index=3}
